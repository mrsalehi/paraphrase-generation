...
class SubwordTextEncoder(TextEncoder):
    ...
    def encode(self, s, disable_tokenizer=False):
        ...
        if disable_tokenizer:
            ...
            tok_list = tokenizer.encode(native_to_unicode(s))

        return self._tokens_to_subtoken_ids(tok_list)
    ...
    def decode(self, ids, strip_extraneous=False, disable_tokenizer=False):
        ...
        if strip_extraneous:
            ids = strip_ids(ids, list(range(self._num_reserved_ids or 0)))
        if disable_tokenizer:
            return self._subtoken_ids_to_tokens(ids)
        else:
...
if __name__ == '__main__':
    tsv = read_tsv('../../data/quora_naug/train.tsv')
    strs = []
    for row in tsv:
        strs.extend([s.split() for s in row])

    gen = iter(strs)

    encoder = SubwordTextEncoder.build_from_generator(
        gen,
        target_size=300,
